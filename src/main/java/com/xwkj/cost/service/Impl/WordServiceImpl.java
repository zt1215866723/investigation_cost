...
public class WordServiceImpl implements WordService{
	...
	@SneakyThrows
	@Override
	public void printfContractWord(String[] contractId, HttpServletResponse response) {
		...
		{
			ContractInfoAndInvoiceInfoVo contractInfoAndInvoiceInfoVo = (ContractInfoAndInvoiceInfoVo)list.get(i);
	...
	public void invoiceStatisticsExcel(SelectContractInfoVo selectContractInfoVo, HttpServletResponse response) {
		...
		if ((selectContractInfoVo.getStart() != null) && (selectContractInfoVo.getEnd() != null)) {
			cell.setCellValue("廊坊市城市建设勘察院发票统计表（" + DateUtil.dateToString(selectContractInfoVo.getStart(), "yyyy-MM") + "-" + DateUtil.dateToString(selectContractInfoVo.getEnd(), "yyyy-MM") + "）");
		} else {
	...
	public static File putBatchFilesInZip(List<String> filePaths,File tempFile) throws IOException {
		...
		for (String pathFile : filePaths)
		{
			...
			try
			{
				FileInputStream fis = new FileInputStream(inputFile);Throwable localThrowable3 = null;
				try
				{
